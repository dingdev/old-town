...
/**
 * MyBatisPlus代码生成器
 * Created by dyp on 2020/8/20.
 */
public class MyBatisPlusGenerator {

    public static void main(String[] args) {
        String projectPath = System.getProperty("user.dir");
        String moduleName = scanner("模块名(!!! 请先检查templateConfig和override的设置是否正确,防止覆盖已有代码 !!!)");
        String[] tableNames = scanner("表名，多个英文逗号分割或者模块名_* ").split(",");
        // 代码生成器
    ...
    private static GlobalConfig initGlobalConfig(String projectPath) {
        ...
        globalConfig.setAuthor("dyp");
        globalConfig.setOpen(false);
        globalConfig.setSwagger2(true);
        globalConfig.setBaseResultMap(true);
        // 是否覆盖,配合templateConfig使用
    ...
    private static TemplateConfig initTemplateConfig() {
        ...
        ////可以对controller、service、entity模板进行自定义,注意不要带上.ftl/.vm, 会根据使用的模板引擎自动识别
        templateConfig.setController("templates/controller2.java");
        templateConfig.setService("templates/service2.java");
        templateConfig.setServiceImpl("templates/serviceImpl2.java");
        templateConfig.setMapper("templates/mapper2.java");
        templateConfig.setEntity("templates/entity2.java");
        ////修改表时设为null则不会再生成,xml已自定义,配合globalConfig.setFileOverride()使用
    ...
    private static InjectionConfig initInjectionConfig(String projectPath, String moduleName) {
        ...
        // 注入自定义配置，可以在 VM 中使用 injectionConfig.pkg
        InjectionConfig injectionConfig = new InjectionConfig() {
            @Override
            public void initMap() {
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("pkg",packageBase.substring(0,packageBase.lastIndexOf(".")));
                this.setMap(map);
            }
        };

        // 模板引擎是Velocity
        String templatePath = "/templates/mapper2.xml.vm";
        // 自定义输出配置
